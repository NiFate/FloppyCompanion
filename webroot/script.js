...
async function init() {
    ...
    // --- Populate Kernel Links based on device ---
    ...
    if (kernelLinksCard && kernelLinksList) {
        ...
        if (device && device.schemaKey === 'features_1280') {
        ...
        } else if (device && device.schemaKey === 'features_trinket') {
    ...
    function updateBottomPadding(hasApplyButton) {
        ...
        const lastContainer = switchContainers[switchContainers.length - 1];
        if (lastContainer) {
